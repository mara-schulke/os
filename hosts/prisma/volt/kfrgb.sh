...
function set_mode() {
	...
	check_hex_values "${fury_begin_trnsfer} ${fury_reg_apply} ${fury_end_trnsfer} ${set_mode_to} ${mode_hex}"
	...
	for ramstick_hex in ${ramsticks_hex//,/$' '}; do
		...
		sleep "${wait}"
		i2cset_retry -y "${smbus_number}" "0x${ramstick_hex}" "0x${fury_reg_apply}" "0x${fury_begin_trnsfer}"
		...
		i2cset_retry -y "${smbus_number}" "0x${ramstick_hex}" "0x${fury_reg_apply}" "0x${fury_end_trnsfer}"
